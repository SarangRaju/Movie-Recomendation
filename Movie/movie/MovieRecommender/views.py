...
def generateRecommendation(request):
    ...
    #Movie Data Frames
    for item in movie:
        x=[item.id,item.title,item.description,item.actors,item.movieduration,item.image.url,item.genres]
        y+=[x]
    movies_df = pd.DataFrame(y,columns=['movieId','title','desciption','actors','movieduration','image','genres'])
